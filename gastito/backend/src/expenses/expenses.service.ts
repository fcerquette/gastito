...
export class ExpensesService {
  ...
  async listByGroup(groupId: string, userId: string): Promise<Expense[]> {
    ...
    return this.expensesRepo.find({
      ...
      relations: ['splits', 'splits.groupMember', 'splits.groupMember.user', 'paidBy'],
    });
  }
  ...
  async create(
    ...
    // Verificar que paidById sea un user del grupo
    const payerMember = await this.membersRepo.findOne({
      where: { groupId, userId: dto.paidById },
    });
    if (!payerMember)
      throw new BadRequestException('Payer must be a registered member of the group');
  ...
  async remove(id: string, userId: string): Promise<void> {
    ...
    if (expense.paidById !== userId)
      throw new ForbiddenException('Only the payer can delete the expense');
